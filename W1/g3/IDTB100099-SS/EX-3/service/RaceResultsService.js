...
export class RaceResultsService {
  ...
  loadFromFile(filePath) {
    ...
    try {
      ...
      this._raceResults = results.map((result) => {
  ...
  getTimeForParticipant(participantId, sport) {
       // TODO
       const result = this._raceResults.find((result) => {
        return result.participantId === participantId && result.sport === sport;
      });
  ...
  getTotalTimeForParticipant(participantId) {
        // TODO
        const results = this._raceResults.filter((result) => {
        ...
        const totalSeconds = results.reduce((total, result) => {
          return total + result.time._totalSeconds;
        }, 0);
